...
class HCOCO(data.Dataset):
    def __init__(self, mode, args=None, sample=[], gan_norm=False):
        ...
        print('total Dataset of ' + self.dataset + ' is :s ', len(self.train))
    ...
    def __getitem__(self, item):
        ...
        # ori_mask = cv2.imread(self.mask[item])
        # mask = mask[:, :, 0].astype(np.float32) / 255.
        # mask = mask.astype(np.uint8)
        # mask = Image.fromarray(mask)
        # ori_mask = ori_mask[:, :, 0].astype(np.float32) / 255.
        # ori_mask = ori_mask.astype(np.uint8)
        # ori_mask = cv2.resize(ori_mask, (256, 256))

        anno = Image.open(self.anno[item]).convert('RGB')
        # cv2_tgt = cv2.imread(self.anno[item])
        # cv2_tgt = cv2.cvtColor(cv2_tgt, cv2.COLOR_BGR2RGB)
        # cv2_tgt = cv2.resize(cv2_tgt, (256, 256))
        ...
        if self.isTrain and random.random() > 0.5:
            img,anno,mask,thumb_foreground,thumb_mask = [ item.transpose(Image.FLIP_LEFT_RIGHT) for item in [img,anno,mask,thumb_foreground,thumb_mask]]
        ...
        return {"composite_images": self.lr_trans(img),
                ...
                "ori_mask": self.hr_trans(mask),
                "ori_tar":  self.hr_trans(anno),
                "cv2_img": self.trans_tensor(cv2_img),
