...
def CF(img, param, pieces):
    ...
    total_image = img
    for i in range(pieces):
        total_image += param[:, :, :, :, i] * (torch.pow(total_image, 2) - total_image)
    return total_image
...
class CurveRenderwithConcat(nn.Module):
    def __init__(self, plane=64, final_relu=False, p=Projection):
        ...
        self.plane = plane
        self.fc_f_1 = p(512, 3 * self.plane, final_relu)  # bs x 3 x 64 #
        self.fc_b_1 = p(512, 3 * self.plane, True)  # bs x 3 x 64 #
        self.fc_b_2 = p(3 * self.plane, 3 * self.plane, final_relu)  # bs x 3 x 64 #
    ...
    def forward(self, x, f_feature, b_feature, label=None):
        ...
        param = self.fc_b_1(b_feature)
        param = self.fc_b_2(param)
...
class CreateNetFusionwoMask(nn.Module):
    ...
    def forward(self, ori_img, x, fore, label, withLabel):
        ...
        separate_feature = torch.cat([fx * fm, x * (1 - m)], dim=0)
        union_feature = torch.cat([x, x], dim=0)
        feature = self.squeezenet1_1(separate_feature)
        # import pdb;
        # pdb.set_trace()
        f_feature, b_feature = torch.split(feature, feature.size(0) // 2, dim=0)
