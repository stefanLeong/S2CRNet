...
class UnetSplit(nn.Module):
    def __init__(self, in_channels=3, depth=5, shared_depth=0, blocks=1,
                 out_channels_image=3, out_channels_mask=1, start_filters=32, residual=True, batch_norm=nn.BatchNorm2d,
        ...
        if self.shared != 0:
            ...
            self.shared_decoder = UnetDecoderD(in_channels=start_filters * 2 ** (depth - 1),
                                               ...
                                               batch_norm=batch_norm, transpose=transpose, concat=concat, is_final=False)
