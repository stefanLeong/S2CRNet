...
def mobilenet(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> MobileNetV2:
    """
    ...
    if pretrained:
        state_dict = load_url(model_urls["mobilenet_v2"], progress=progress)
...
class CurveRenderwithConcat(nn.Module):
    def __init__(self, plane=128, final_relu=False, p=Projection):
        ...
        self.plane = plane
        self.fc_f_1 = p(96, 3 * self.plane, final_relu)  # bs x 3 x 64 #
        self.fc_b_1 = p(96, 3 * self.plane, final_relu)  # bs x 3 x 64 #
        print(self.plane)

        self.fc_label_1 = nn.Linear(5, 32)
        self.fc_label_2 = nn.Linear(32, 64)
        self.fc_f_label = nn.Linear(96 + 64, 3 * self.plane, bias=True)
