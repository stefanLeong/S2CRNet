...
from scripts.utils.osutils import mkdir_p, is_file, is_dir, join_path
    ...
    def train(self, epoch):
        ...
        for _ in range(self.hl):
            for i, batches in enumerate(self.train_loader):
                ...
                # plot progress
                suffix = '({batch}/{size}) Data: {data:.2f}s | Batch: {bt:.3f}s | Total: {total:} | ETA: {eta:} | Loss L2: {loss_label:.4f} '.format(
                    batch=i + 1,
                ...
                if current_index % self.args.freq == 0:
    ...
    def validate(self, epoch):
        ...
        with torch.no_grad():
            for i, batches in enumerate(self.val_loader):
                ...
                output = im_to_numpy(torch.clamp(output * 255, min=0.0, max=255.0)).astype(np.uint8)
                target = im_to_numpy(torch.clamp(target * 255, min=0.0, max=255.0)).astype(np.uint8)
                psnr_loss = psnr(target, output, data_range=output.max() - output.min())

                losses.update(L2_loss.item(), inputs.size(0))
                psnres.update(psnr_loss, inputs.size(0))
