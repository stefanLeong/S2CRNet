    ...
    def train(self, epoch):
        ...
        for i, batches in enumerate(self.train_loader):
            ...
            if self.withLabel:
                ...
                label_oh = None
            outputs, param1, param2, param3, param4 = self.model(feeded, feeded, fore, label_oh, self.withLabel)
    ...
    def test(self, ):
        ...
        with torch.no_grad():
            for i, batches in enumerate(self.val_loader):
                ...
                # ori_tag = batches["ori_tar"].to(self.device)
                # unresize_img = batches["no_resize"].to(self.device)
                # print(label.shape)
                label_oh = torch.zeros(self.args.test_batch, 5).scatter_(1, label.view(label.shape[0], 1), 1).to(
                    torch.float32).to(self.device)
                if self.withLabel:
                ...
                start.record()
                # resize = self.resize_trans(unresize_img)
                outputs, param1, param2 = self.model(ori_img, feeded, fore, label_oh, self.withLabel)

                # ori_img = F.interpolate(ori_img, size=(512, 512), mode='bilinear')
                end_time.record()
                ...
                imfinal = outputs[0] * mask + inputs * (1 - mask)
                # imfinal = outputs[0] * ori_mask + ori_img * (1 - ori_mask)
                ...
                # lpips_score = loss_fn_vgg(imfinal.cpu(), target.cpu())
                ...
                fmse = compare_mse(imfinal * mask.numpy(), target * mask.numpy()) * 256 * 256 / (
                    torch.count_nonzero(mask))
                print(macs, params, start.elapsed_time(end_time), fmse)
                ...
                times.update(start.elapsed_time(end_time), inputs.size(0))
                psnres.update(psnr, inputs.size(0))
                ssimes.update(ssim, inputs.size(0))
                mses.update(mse, inputs.size(0))
