...
from scripts.utils.misc import resize_to_match, get_dims_with_exclusion
...
def fMSE(pred, target_image, mask):
    ...
    return np.float64((diff.sum() / (diff.size(2) * mask.sum() + 1e-6)).item())
...
def MaskWeightedMSE(min_area, pred, target, mask):

    target = target.view(pred.size())
    reduce_dims = get_dims_with_exclusion(target.dim(), 0)

    loss = (pred - target) ** 2
    delimeter = pred.size(1) * torch.clamp_min(torch.sum(mask, dim=reduce_dims), min_area)
    ...
    def forward(self, imgx, target, attx, mask, wmx, wm):
        ...
        if is_dic(imgx):
            ...
            if self.args.masked:
                ...
                pixel_loss = self.outputLoss(imgx[0], target) + sum(
                    [self.outputLoss(im, resize_to_match(mask, im) * resize_to_match(target, im)) for im in imgx[1:]])
            else:
                pixel_loss = sum([self.outputLoss(im, resize_to_match(target, im)) for im in imgx])

            if self.args.style_loss > 0:
                vgg_loss = sum(
                    [self.vggloss(im, resize_to_match(target, im), resize_to_match(mask, im)) for im in imgx])
    ...
    def forward(self, x, y, Xmask=None):
        ...
        for i in range(3):
            if self.relative:
                ...
                loss += self.criterion(resize_to_match(mask, x_vgg[i]) * x_vgg[i],
                                       resize_to_match(mask, y_vgg[i]) * y_vgg[i].detach())
